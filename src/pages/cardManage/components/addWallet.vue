<template>
  <view class="addWallet">
    ...
    >
      <view class="title">
        <text>{{ $t("cardManage.addWallet.title") }}</text>
        <uni-icons
          class="titleIocn"
          type="closeempty"
          size="20"
      ...
      <view class="set-amount">
        ...
        >
          <view class="formView">
            <uni-forms-item
              :label="$t('cardManage.addWallet.type')"
              name="type"
            >
              <uni-data-select
                v-model="prarms.type"
                :localdata="range"
                :clear="false"
                :placeholder="$t('cardManage.addWallet.type')"
              ></uni-data-select>
            </uni-forms-item>
            <uni-forms-item
              :label="$t('cardManage.addWallet.account')"
              name="account"
            >
              <uni-easyinput
                class="easy-inp"
                v-model="prarms.account"
                :placeholder="$t('cardManage.addWallet.account')"
              />
            </uni-forms-item>
            <uni-forms-item
              :label="$t('cardManage.addWallet.address')"
              name="addr"
            >
              <uni-easyinput
                class="easy-inp"
                v-model="prarms.address"
                :placeholder="$t('cardManage.addWallet.address')"
              />
            </uni-forms-item>
      ...
      rules: {
        type: {
          rules: [
            {
              required: true,
              errorMessage: this.$t("cardManage.addWallet.type"),
            },
          ],
        },
        account: {
          rules: [
            {
              required: true,
              errorMessage: this.$t("cardManage.addWallet.account"),
            },
          ],
        },
        address: {
          rules: [
            {
              required: true,
              errorMessage: this.$t("cardManage.addWallet.address"),
            },
          ],
        },
        payPwd: {
