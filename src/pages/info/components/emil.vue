      ...
      <uni-collapse-item class="info-card">
        <template #title>
          <view class="card-title">
            <text class="title-left">{{ $t("info.email.title") }}</text>
            <text class="title-right">{{
              formatEmail(vuex_user.email ? vuex_user.email : "")
            }}</text>
          </view>
        </template>
        <view class="card-content">
          <uni-forms
            ...
            :rules="realRules"
          >
            <uni-forms-item :label="$t('info.email.email')" name="email">
              <uni-easyinput
                class="input"
                v-model="realData.email"
                type="text"
                :placeholder="$t('info.email.emailplace')"
              />
            </uni-forms-item>

            <uni-forms-item
              class="last-item"
              :label="$t('info.email.code')"
              name="code"
            >
              <uni-easyinput
                class="input"
                ...
                type="text"
                :placeholder="$t('info.email.codeplace')"
              />
              <button v-show="btnShow" class="code-btn" @click="send">
                {{ $t("info.real.codebtn") }}
              </button>
              <button class="code-btn" disabled="true" v-show="timeshow">
                {{ times }}
              </button>
            </uni-forms-item>
          </uni-forms>
...
export default {
  ...
  data() {
    return {
      ...
      btnShow: true,
      typesName: "asdjdjsj ",
      fomatemail: this.$t("info.email.fomatemail"),
      realData: {
        email: "",
        code: "",
      },
      realRules: {
        email: {
          rules: [
            {
              required: true,
              errorMessage: this.$t("info.email.emailplace"),
            },
            {
              format: "email",
        ...
        code: {
          rules: [
            { required: true, errorMessage: this.$t("info.email.codeplace") },
          ],
        },
      },
      areaCode: 91,
  ...
  methods: {
    ...
    submit() {
      this.$refs.form.validate().then((res) => {
        this.$api
          .mailBind(this.realData)
          .then((res) => {
            this.loadPhoen(this.realData.email);
            uni.showToast({
    ...
    loadPhoen(email) {
      this.$updateStore("vuex_user.email", email);
    },
    send() {
      var re = /\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*/;
      if (this.realData.email == "" || !re.test(this.realData.email)) {
        uni.showToast({
          title: this.$t("info.email.emailplace"),
          icon: "error",
        });
        return;
      }
      ...
      that.timeshow = true;
      that.btnShow = false;
      this.$api.mailCode({ email: this.realData.email }).then((res) => {
        uni.showToast({
          title: this.$t("info.email.codesuccess"),
        });
        if (that.times == 60) {
